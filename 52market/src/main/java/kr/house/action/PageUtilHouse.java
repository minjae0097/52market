...
public class PageUtilHouse {
	 ...
	 * */
	public PageUtilHouse(int currentPage,int count, int rowCount,int status) {
		...
		this(null,null,currentPage,count,rowCount,0,null,status,0,0,0,0);
	}
	
	public PageUtilHouse(String keyfield, String keyword, int currentPage, int count, int rowCount,
			int pageCount,String pageUrl,int status,int house_seller_type,int house_type,int house_deal_type,int house_move_in) {

		if(count >= 0) {
			String sub_url = "";
			if(keyword != null || !"".equals(keyword)) sub_url = "&keyfield="+keyfield+"&keyword="+keyword;
			if(house_seller_type >= 1 && house_seller_type <= 9) sub_url += "&house_seller_type="+house_seller_type;
			if(house_type >= 1 && house_type <= 9) sub_url += "&house_type="+house_type;
			if(house_deal_type >=1 && house_deal_type <= 9) sub_url += "&house_deal_type="+house_deal_type;
			if(house_move_in >=1 && house_move_in <= 9) sub_url += "&house_move_in="+house_move_in;
